...
- `hard` 幸存者会主动进攻(不是纯逃跑)。
- **僵尸互斥 (R1)**:僵尸经移动/召唤/感染生成都不得占据与另一僵尸正交相邻的格(`rules.ts::zombieMayOccupy` 单一约束源)。破"不可摧贴边墙"规则缺陷:满列贴边僵尸 100% 免疫跳杀 → 强制平局。不破坏感染(夹同一幸存者的 2 僵尸恒非正交相邻,S-3 守卫;S-4 守卫证实墙已可破)。
- **感染生成异地化 (R2)**:被感染者不在原地复活僵尸,改在"离最近存活幸存者最近的 R1-合法点"**确定性**生成(`pickInfectionSpawn`);无可用点/无存活幸存者则跳过。
- **僵尸 AI = 反龟缩激进组**(`zombieScore` n1=140/n2=360/pincer=30/minD\*5/killable useful=150;`zombieOffense` 160/340/\*12/\*26/70-34)。harness 实测:僵局 hard/hard 59%→~8%、easy/easy 4%→0%(龟缩/退化平局根除)。**已知架构天花板**:survivor 跳杀是 10⁵-scale,hard 的 worst-case minimax 让纯权重**调不出 ~50:50**(easy/easy、hard/hard 幸存者 ~88%;hardZ/easyS≈50:50)。这是有意取舍(用户拍板:优先反龟缩)。真要 ~50:50 需改 hard `scoreAction` 的 worst-case 架构(单独立项,见 `docs/superpowers/specs/2026-05-16-zomboy-zombie-exclusion-design.md`)。

改动这些前先问用户,并说明会破坏什么平衡。完整设计/证明见 `docs/superpowers/specs/2026-05-16-zomboy-zombie-exclusion-design.md`。
